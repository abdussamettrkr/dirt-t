...
class Mnist(object):
    def __init__(self):
        ...
        print("Loading MNIST")
        print(os.getcwd())
        train = loadmat(os.path.join(args.datadir, 'source_train.mat'))
        test = loadmat(os.path.join(args.datadir, 'source_test.mat'))

        trainx = s2t(train['X'])
        trainy = train['y'].reshape(-1)
        trainy = np.eye(2)[trainy].astype('float32')

        testx = s2t(test['X'])
        testy = test['y'].reshape(-1)
        testy = np.eye(2)[testy].astype('float32')

        trainx = trainx.reshape(-1, 32, 32, 3).astype('float32')
        testx = testx.reshape(-1, 32, 32, 3).astype('float32')

        self.train = Data(trainx, trainy)
        print('Mnist train samples:',np.shape(trainx),np.shape(trainy))
        self.test = Data(testx, testy)
        print('Mnist test samples:',np.shape(testx),np.shape(testy))
...
class Svhn(object):
    def __init__(self, train='train'):
        ...
        """
        print("Loading SVHN")
        train = loadmat(os.path.join(args.datadir, 'target_train.mat'))
        test = loadmat(os.path.join(args.datadir, 'target_test.mat'))
        ...
        self.train = Data(trainx, trainy)
        print('Svhn train samples:',np.shape(trainx),np.shape(trainy))
        self.test = Data(testx, testy)
        print('Svhn test samples:',np.shape(testx),np.shape(testy))
